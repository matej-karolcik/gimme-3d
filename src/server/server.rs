...
async fn serve(port: u16, request_tx: mpsc::Sender<(request::Request, ResultChannel)>) {
    ...
    let render_form = warp::post()
        ...
        .and_then(
            |form: FormData,
             ...
             sem: Arc<Semaphore>,
             request_tx: mpsc::Sender<(request::Request, ResultChannel)>| async move {
